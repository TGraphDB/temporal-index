...
import static org.act.temporal_index.util.Slices.*;
    ...
    NEW_FILE(7)
            {
                ...
                public void writeValue(SliceOutput sliceOutput, VersionEdit versionEdit)
                {
                    for (Entry<Integer, IndexMetaData> entry : versionEdit.getNewFiles().entries()) {
                        VariableLengthQuantity.writeVariableLengthInt(getPersistentId(), sliceOutput);
                        ...
                        // file number
                        IndexMetaData fileMetaData = entry.getValue();
                        ...
                        // smallest key
                        Slice sk = new Slice( 4 );
                        sk.setInt( 0, fileMetaData.getSmallest() );
                        writeLengthPrefixedBytes(sliceOutput, sk);

                        // smallest key
                        Slice lk = new Slice( 4 );
                        lk.setInt( 0, fileMetaData.getLargest() );
                        writeLengthPrefixedBytes(sliceOutput, lk );
                    }
                }
            };
